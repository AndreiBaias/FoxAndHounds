...
#algoritmul minimax aplicat pentru vulpe, analog celui anterior dar cu functia de evaluare schimbata
def minmax_fox(position, depth, max_player):
    if depth == 0 or position.winner() != None:
    ...
    if max_player:
        ...
        best_move = None
        for move in get_all_moves(position, RED):
            evaluation = minmax_fox(move, depth-1, False)[0]
            maxEval = max(maxEval, evaluation)
            if maxEval == evaluation:
                best_move = move
        ...
        best_move = None
        for move in get_all_moves(position, BLACK):
            evaluation = minmax_fox(move, depth - 1, True)[0]
            minEval = min(minEval, evaluation)
            if minEval == evaluation:
                best_move = move
...
def depth_selector_player_ai():
    ...
    while intro1:
        ...
        # playerVSaiText = window.blit(playerVSai, playerVSai.get_rect())  # title is an image
        # playerVSaiText.center = ((WIDTH / 2), (HEIGHT / 2))
...
def main_ai(depth):
    ...
    while running:
        ...
        if game.turn == BLACK:
            ...
            value, new_board = minmax_fox(game.get_board(), depth, True)
            game.ai_move(new_board)
